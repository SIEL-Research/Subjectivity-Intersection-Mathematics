...
\hypersetup{
  ...
  citecolor=SIELblue,
  urlcolor=SIELblue,
  pdftitle={Subjectivity-Intersection Mathematics: Constitutive Closure and Stabilized O3 Return across Atomic, Molecular, and Cellular Systems},
...
\fancyhead[R]{\small\sffamily Public Working Preprint v4.2 Draft}
...
Constitutive Closure and Stabilized O3 Return\\across Atomic, Molecular, and Cellular Systems\par}
\vspace{7mm}
{\sffamily\fontsize{15}{19}\selectfont
Formal Criteria, Failure-Guided Cross-Domain Tests,\\and Domain Boundaries\par}
...
\noindent\colorbox{SIELlight}{%
  ...
  \vspace{2mm}
  \textbf{Document status:} Public Working Preprint, Version 4.2 Draft\\
  \textbf{Date:} 13 August 2026\\
  \textbf{DOI:} \href{https://doi.org/10.5281/zenodo.21909933}{10.5281/zenodo.21909933}\\
  \textbf{Preceding public version DOI:} \href{https://doi.org/10.5281/zenodo.21877695}{10.5281/zenodo.21877695}\\
...
Two preregistered precursor tests rejected uniform absolute calibration while preserving a common causal ordering. E014 therefore tested the refined invariant directly across independent reduced atomic, molecular, and cellular engines without a shared scalar mediator: after a domain-native stabilization interval, correct O3 return must be closer to the registered domain state than continued removal or physically distinguishable mismatched return. This direction passed all atomic ($3/3$), molecular ($9/9$), and cellular ($8/8$) primary checkpoints. The evidence supports a cross-domain causal invariant of stabilized O3-return specificity over the tested realizations, not a common O3 substance, common metric, microscopic atom-to-cell law, phenomenal subjecthood, or living-cell confirmation.
\end{abstract}
...
\begin{enumerate}[itemsep=0.1em,topsep=0.25em]
  \item The primary mathematical object is generalized from reciprocal perspective intersection to a domain-relative constitutive-closure structure with explicit references, interventions, re-entry, whole-formation criteria, and representation equivalence.
  \item A common grammar maps differentiated operational perspectives to a generated third term and its return: $(O_1^D,\ldots,O_n^D)\rightarrow C_D\rightarrow(O_1^{D\prime},\ldots,O_n^{D\prime},C_D')$.
  ...
  \item Experiment 011 is retained as a held-out representation-covariance test inside a frozen reduced bridge, not an independent atom--molecule--cell test.
  \item Experiments 012 and 013 preserve two preregistered failures: mismatch specificity was representation-sensitive, and a common absolute score threshold did not transfer.
  \item Experiment 014 prospectively supports a narrower cross-domain invariant across independently executed targets: after domain-native stabilization, correct O3 return is closer to the registered state than removal or mismatched return.
  \item A compression ladder limits inference from domain construction through intervention and readout to the shared sign invariant; scalar agreement is not inverted into common source identity.
...
\subsection{Operational perspective is not a replaceable label}
...
\end{equation}
where $L_i$ is a differentiated local carrier, $S_i$ is the state space distinguishable from its relational position, $T_i$ is the class of transformations admissible at that position, and $Q_i$ is the registered causal readout available from it. Thus $L_i$ carries a perspective but is not identical to it. The word \emph{perspective} marks the organized selectivity of distinctions, transformations, and consequences. It does not attribute consciousness or phenomenal experience to particles, molecules, cellular modules, or numerical variables.

Let
\begin{equation}
 \mathbf O_D(t)=\bigl(O_1^D(t),\ldots,O_n^D(t)\bigr).
\end{equation}
A constitutive intersection map $\mathcal X_D$ and re-entry map $\mathcal E_D$ define
\begin{equation}
 \mathbf O_D
...
\end{equation}
where $\mathcal R_D$ constructs matched isolated or local-only references; $C_D$ is the joint term generated relative to those references; $\mathcal I_D$ is a preregistered class of admissible interventions; $\mathcal E_D$ returns the candidate relation through unchanged or explicitly frozen dynamics; $W_D$ is a vector of whole-formation readouts with a registered decision region $\Omega_D$; and $\sim_D$ is an equivalence relation over admissible representations.

The use of a readout vector rather than an unrestricted binary label prevents the definition from installing the desired whole in advance. The experiment freezes measurable coordinates $W_D$, their acceptance region $\Omega_D$, and relevant nulls before the confirmatory intervention. For state $z=(\mathbf O_D,C_D)$, write
\begin{equation}
...
\subsection{Decision conditions}
...
\begin{enumerate}
  \item \textbf{Generation.} $C_D=\mathcal X_D(\mathbf O_D;\mathcal R_D)$ is absent from the matched isolated construction and is not installed as a target, label, or unexplained primitive.
  \item \textbf{Necessity.} For the registered destructive intervention $I^-\in\mathcal I_D$, the whole-formation decision or specified continuous readout is lost:
  \begin{equation}
  ...
  \item \textbf{Re-entry.} Returning the same candidate through the frozen relation and timing restores the registered readout:
  \begin{equation}
    \chi_D\!\left(\mathcal E_D(I^-(\mathbf O_D,C_D),C_D)\right)=1.
    \label{eq:reentry-condition}
  \end{equation}
  \item \textbf{Specificity.} Equal-norm substitutes, cross-pair exchange, time shifts, incomplete sector returns, or other registered false returns do not reproduce the same restoration.
  \item \textbf{Registered empirical minimality.} No tested proper subterm $c\subsetneq C_D$ reproduces the complete vector of necessity, bilateral or all-part return, reconstruction, and whole-formation results. This criterion applies only within the registered intervention class $\mathcal I_D$ and is not proof of absolute mathematical uniqueness.
  \item \textbf{Representation covariance.} For every registered admissible representation map $g$, there exists a transformed realization $g(C_D)$ such that
  \begin{equation}
...
Null discrimination is not a seventh decorative item but a requirement across all six conditions. Additive coupling, correlation, a latent mode, an order parameter, or an interaction Hamiltonian can be important without satisfying generation, selective loss, specific return, and covariance together. Conversely, not every domain experiment presently tests the maximal definition. Table~\ref{tab:evidence-classes} states the evidence level actually attained.

\subsection{Cross-domain evidence gates}

The structure in Equation~\eqref{eq:closure-structure} distinguishes constitutive closure from an arbitrary decomposition followed by intervention in four ways. References and admissible interventions are frozen; the proposed third term must be generated rather than merely named; destruction must be paired with specific return rather than effect alone; and identity is attached to a preserved intervention--re-entry structure rather than a preferred coordinate. Each experiment below freezes its own bridge, target, controls, and interpretation boundary. Negative, mixed, unresolved, and open outcomes remain part of the same record.

Within-domain closure confirmation and cross-domain transfer are distinct decisions. Let $X_D^0(t)$ denote the registered intact domain state, and let $X_D^{+}(t)$, $X_D^{-}(t)$, and $X_D^{\times}(t)$ denote states following correct O3 return, continued O3 removal, and physically distinguishable mismatched return. Each domain preregisters its own similarity or recovery readout $S_D$, stabilization time $\tau_D$, and checkpoint set $T_D$. Define
\begin{align}
 \Delta_D^{-}(t)
 &=S_D\!\left(X_D^{+}(t),X_D^0(t)\right)
   -S_D\!\left(X_D^{-}(t),X_D^0(t)\right),\\
 \Delta_D^{\times}(t)
 &=S_D\!\left(X_D^{+}(t),X_D^0(t)\right)
   -S_D\!\left(X_D^{\times}(t),X_D^0(t)\right).
 \label{eq:stabilized-margins}
\end{align}
The proposed stabilized O3-return signature is
\begin{equation}
 \sigma_D(t)=
 \left(\operatorname{sgn}\Delta_D^{-}(t),
       \operatorname{sgn}\Delta_D^{\times}(t)\right)=(+,+),
 \qquad t\in T_D,\ t\geq\tau_D.
 \label{eq:sign-invariant}
\end{equation}
Cross-domain transfer of this signature requires Equation~\eqref{eq:sign-invariant} to pass prospectively in every registered domain. The values of $S_D$, the state coordinates, the carrier, the stabilization interval, and the margin magnitudes need not be equal. What is compared is the preregistered causal direction after domain-native stabilization. This is an operational sufficient condition over the tested realizations, not a universal metaphysical necessity.

The inference boundary is represented by the compression ladder
\begin{equation}
 \begin{aligned}
 \text{source construction}
 &\longrightarrow \text{O3 carrier or generator}
 \longrightarrow \text{intervention history}\\
 &\longrightarrow \text{domain state}
 \longrightarrow \text{registered readout}
 \longrightarrow \sigma_D.
 \end{aligned}
 \label{eq:compression-ladder}
\end{equation}
Evidence normally travels from left to right. Agreement at a scalar readout or at $\sigma_D$ does not license inversion to identity of source construction, physical substance, or microscopic mechanism. Conversely, prospective agreement of the causal direction across independently executed engines is stronger than numerical coincidence inside one shared scalar mediator.
...
\subsection{Scientific representation, perspective, and invariance}

Scientific representation has been analyzed through similarity, structural mappings, modeler purposes, and perspectival representation~\cite{giere2006,vanfraassen2008,friggnguyen2020}. These traditions already show that a model does not transparently copy a target and that invariants can matter more than surface coordinates. SIM does not claim the discovery of invariance or perspective. It combines them in a stricter constitutive question: which representation changes preserve the registered generation, intervention, and re-entry identity of a third perspective?

Operational perspective in SIM is domain-relative but not merely observer-relative. Equation~\eqref{eq:domain-perspective} binds a local carrier to its accessible distinctions, admissible transformations, and causal readouts. This makes perspective part of the formal bridge. Removing it would convert the theory into a component decomposition and conceal the proposed explanatory reversal.

\subsection{Intervention and causal explanation}

Interventionist accounts characterize causal relations through changes under suitable manipulations and connect explanation to invariance~\cite{pearl2000,woodward2003}. SIM inherits that counterfactual discipline but does not infer constitution from intervention sensitivity alone. Many causally relevant variables can be changed without constituting the identity of a whole. The added requirements are generation relative to an isolated reference, non-reduction, specific return, registered empirical minimality, and representation covariance. ``Constitutive'' therefore names a constrained pattern of counterfactual evidence, not a synonym for ``causally influential.''

\subsection{Relational physics and standard decomposition}

Relative coordinates and reduced mass are standard two-body mechanics, and relational interpretations of physical state long predate this work~\cite{landau1977,rovelli1996}. Likewise, quantum chemistry contains mature families of energy, orbital, fragment, and atoms-in-molecules decompositions~\cite{morokuma1971,kitaura1976,bickelhaupt2000,stone2013}. SIM claims neither the discovery of the reduced coordinate nor decomposition itself.
...
\subsection{Autopoiesis and organizational closure}

Autopoiesis defines living organization through a network that produces and realizes the unity of its own components~\cite{varela1974,maturana1980}. Organizational and constraint-closure approaches further analyze mutually dependent constraints, self-maintenance, autonomy, and biological function~\cite{mossio2009,mossio2010,montevil2015,moreno2015}. E009 is not presented as the first closure account of life and does not establish autopoiesis in a living cell.
...
\subsection{Boundary of the SIM contribution}

Table~\ref{tab:prior-boundary} makes the novelty claim explicit. The mathematical ingredients have precedents; the proposed contribution is their organization around perspective intersection and prospectively testable constitution.

\begin{table}[H]
\centering
\scriptsize
\caption{Neighboring concepts and the additional SIM commitment.}
...
\noindent\textbf{Implementation-specific neighboring concepts.}
The originating recurrent-perspective experiments were implemented with two computational agents and must also be separated from familiar agent-model constructs.
...
\subsection{Shared memory and an installed third computational agent}
...
\section{Operational foundation: staged recurrent-perspective experiments}
...
Private MOL-001--MOL-010 exploration fixed the decomposition and intervention logic on two- and three-centre systems. E010 prospectively transferred them to an unexecuted rectangular $H_4^+$ target with four hydrogen centres, charge $+1$, three electrons, and PySCF \texttt{spin=1} (doublet, $2S=1$). Full configuration interaction energies and one-particle density matrices were evaluated across a two-dimensional geometry grid in STO-3G, 6-31G, and cc-pVDZ.
...
\toprule
Basis & $a/b$ at minimum (\AA) & 6~\AA\ landscape depth (Eh) & Removed & Edge displacement (\AA) & Gates \\
\midrule
...
The reported depth is the registered landscape depth relative to the frozen 6~\AA\ reference, not a general dissociation claim. Because the rectangular $H_4^+$ landscape has symmetry-equivalent minima under $a\leftrightarrow b$, edge-deletion displacement is measured to the nearest member of the complete-system equivalent-minimum set; the registered threshold still passes under that convention.
...
\section{Within-bridge precursor: covariance of closure identity in E011}
...
\centering
\footnotesize
\caption{Experiment 011 held-out within-bridge representation predictions.}
\begin{tabular}{@{}rrrrr@{}}
\toprule
...
\begin{center}
\statusnotsupported: \textsf{CROSS-DOMAIN O3 INTERVENTION TRANSFER}.
\end{center}
...
\subsection{Experiment 013: domain-prior generation without common calibration}
...
\begin{equation}
 S_D(X_D^0)>S_D(X_D^-),\quad
 S_D(X_D^0)>S_D(X_D^\times),\quad
 S_D(X_D^+)>S_D(X_D^-),\quad
 S_D(X_D^+)>S_D(X_D^\times).
 \label{eq:ordinal-topology}
\end{equation}
...
\begin{center}
\statusnotsupported: \textsf{DOMAIN-PRIOR O3 GENERATION TRANSFER}.
\end{center}
...
\section{Prospective stabilized O3-return specificity: E014}

\subsection{Registered hypothesis and design}

E014 froze the refinement produced by E012 and E013. For each domain it preregistered a fresh target, a domain-native stabilization interval, an intact reference, removal, correct return, a physically distinguishable mismatch selected without intervention outcomes, and the paired margins in Equation~\eqref{eq:stabilized-margins}. The three engines shared no scalar mediator. The cross-domain claim would fail if either margin were non-positive at any registered primary checkpoint in any domain.

The target engines were hydrogenic helium-4 in an atomic soft-Coulomb relaxation, a fresh H$_4^+$ cc-pVTZ line at $b=1.1$~\AA\ in the molecular engine, and two held-out damage amplitudes in the frozen E009 reduced stochastic cell. The readouts were intentionally domain-specific: Hellinger similarity for the atomic state, registered molecular landscape similarity across grid refinements, and paired multivariate state similarity plus the preregistered recovery boundary for the cellular system.

\subsection{Prospective result}
...
\begin{center}
\statussupported: \textsf{STABILIZED O3 RETURN CROSS-DOMAIN TRANSFER}.
\end{center}
...
\subsection{From representation covariance to causal-direction invariance}
...
\begin{enumerate}
  ...
  \item \textbf{Cross-domain causal invariance:} test whether a preregistered intervention--return direction survives independent domain engines without a common scalar mediator.
\end{enumerate}
...
012 & \href{https://github.com/SIEL-Research/Subjectivity-Intersection-Mathematics/tree/main/experiments/012_cross_domain_o3_intervention_transfer}{Independent cross-domain O3 intervention transfer} & Result: \href{https://doi.org/10.5281/zenodo.21888156}{doi:10.5281/zenodo.21888156}; preregistration: \href{https://doi.org/10.5281/zenodo.21888131}{doi:10.5281/zenodo.21888131} \\
013 & \href{https://github.com/SIEL-Research/Subjectivity-Intersection-Mathematics/tree/main/experiments/013_domain_prior_o3_generation_transfer}{Domain-prior O3 generation transfer} & Result: \href{https://doi.org/10.5281/zenodo.21894771}{doi:10.5281/zenodo.21894771}; preregistration: \href{https://doi.org/10.5281/zenodo.21894665}{doi:10.5281/zenodo.21894665} \\
014 & \href{https://github.com/SIEL-Research/Subjectivity-Intersection-Mathematics/tree/main/experiments/014_stabilized_o3_return_cross_domain_transfer}{Stabilized O3-return cross-domain transfer} & Result: \href{https://doi.org/10.5281/zenodo.21895729}{doi:10.5281/zenodo.21895729}; preregistration: \href{https://doi.org/10.5281/zenodo.21895645}{doi:10.5281/zenodo.21895645} \\
\bottomrule
\end{longtable}

The invalid Experiment 003 record remains preserved at \href{https://doi.org/10.5281/zenodo.21760837}{doi:10.5281/zenodo.21760837}; the Experiment 003R technical addendum is \href{https://doi.org/10.5281/zenodo.21761925}{doi:10.5281/zenodo.21761925}. Experiment 006 remains \statusnotsupported. Experiment 008A retains its standard-contact decision, 008C remains precision-limited, 008D remains unexecuted, and 008E remains an open externally testable prediction. Experiments 012 and 013 retain their preregistered \statusnotsupported\ decisions; their positive component results are not promoted to overall confirmation. Experiment 014 retains its preregistered \statussupported\ decision.
...
\item[Stabilized O3-return specificity] The registered condition that, after a domain-native stabilization interval, correct O3 return is closer to the intact domain state than continued removal or a physically distinguishable mismatched return.
\item[Cross-domain causal-direction invariant] Preservation of the sign vector $(\operatorname{sgn}\Delta_D^{-},\operatorname{sgn}\Delta_D^{\times})=(+,+)$ across independently executed domains; it does not assert equality of scores, carriers, substances, or source mechanisms.
\item[Compression ladder] The one-way evidential sequence from source construction through O3 carrier, intervention history, domain state, and registered readout to a compressed invariant. Agreement at a later level does not by itself identify an earlier-level object.
\item[Evidence class] The explicit status of a result as benchmark-grounded transfer, standard-theory counterfactual, reduced-model confirmation, reduced-bridge prediction, or independent cross-domain intervention.
\end{description}
...
\begin{center}
\small\sffamily
Public Working Preprint v4.2 Draft --- 13 August 2026\\
